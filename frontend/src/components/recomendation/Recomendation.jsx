...
const Recomendation = () => {
  ...
  useEffect(() => {
    dispatch(sortRatingHtL()); // Düzgün async thunk çağırışı
    window.scrollTo(0, 0);
  }, [dispatch]);
        ...
        <div className="recomendation">
          <Title>
            <div className="row p-1">
              <div className="col-6">
                <div className="title-l">Our recommendation books</div>
              </div>
              <div className="col-6" style={{ justifyContent: "flex-end", display: "flex" }}>
                <Link to="/recomendation" className="mainbtn">
                  Explore more
                </Link>
              </div>
            </div>
          ...
          <div className="book-list mt-4">
            {books.slice(0, 9).map((book, index) => (
              <div key={book._id} className="book-item" onClick={() => godetails(book._id)}>
                <img src={book.image} alt={book.title} className="book-image" />
                <div className="count" style={{ fontWeight: "bold", fontSize: "16px" }}>{index + 1}</div>
